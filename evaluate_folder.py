...
def viz(img, flo, flow_dir, id):
    ...
    imageio.imwrite(os.path.join(flow_dir, f'{id:03}.png'), flo)
    print(f"Saving optical flow visualisation at {os.path.join(flow_dir, 'flo.png')}")
...
def batch_folder(args):
    ...
    with torch.no_grad():
        ...
        for id, image_pair in enumerate(zip(images[:-1], images[1:])):
            image1 = load_image(image_pair[0])
            image2 = load_image(image_pair[1])
            #print(f"Reading in images at {imfile1} and {imfile2}")
            ...
            viz(image1, flow_up, flow_dir, id)
